...
class HMM(object):
    ...
    def forward(self, sentence):
        ...
        for i, w in enumerate(sentence):
            if i is 0 :
		for tg in self.tagset :
		    if (tg in self._initial) and (tg in self._emission) and (w in self._emission[tg]) :
                    	alpha[(i,tg)] = self._initial[tg] + self._emission[tg][w]
	    else :
		for cur_tag in self.tagset :
		    alpha[(i, cur_tag)] = dict()
		    for prev_tag in self.tagset :
			if ((i-1, prev_tag) in alpha) and (prev_tag in self._transition and cur_tag in self._transition[prev_tag]) and (cur_tag in self._emission and w in self._emission[cur_tag]) :
			    alpha[(i, cur_tag)].update({prev_tag:alpha[(i-1, prev_tag)] + self._transition[prev_tag][cur_tag] + self._emission[cur_tag][w]})
		    alpha[(i, cur_tag)] = np.log(sum([np.exp(y) for x,y in alpha[(i, cur_tag)].iteritems()]))
		    #print ("DONT", i, cur_tag, alpha[(i, cur_tag)])

	num_words = set([a for a,b in alpha.keys()])
 	for wd in num_words :
	    for tag in self.tagset :
		if (wd, tag) not in alpha :
			alpha[(wd, tag)] = float('-inf')	
    ...
    def build_viterbi_delta(self, sentence):
        ...
        for i, w in enumerate(sentence):
            if i is 0 :
                for tg in self.tagset :
                    if (tg in self._initial) and (tg in self._emission) and (w in self._emission[tg]) :
                        delta[(i,tg)] = self._initial[tg] + self._emission[tg][w]
            else :
                for cur_tag in self.tagset :
                    delta[(i, cur_tag)] = float('-inf')
                    for prev_tag in self.tagset :
                        if ((i-1, prev_tag) in delta) and (prev_tag in self._transition and cur_tag in self._transition[prev_tag]) and (cur_tag in self._emission and w in self._emission[cur_tag]) :
			    p_cur = delta[(i-1, prev_tag)] + self._transition[prev_tag][cur_tag] + self._emission[cur_tag][w]
			    if p_cur > delta[(i, cur_tag)] :
				delta[(i, cur_tag)] = p_cur
				bp[(i, cur_tag)] = prev_tag

        #num_words = set([a for a,b in delta.keys()])
        #for wd in num_words :
        #    for tag in self.tagset :
        #        if (wd, tag) not in delta :
        #                delta[(wd, tag)] = float('-inf')
